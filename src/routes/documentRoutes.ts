...
import type { User, Document as DocumentType } from '../types/database';
import { asyncHandler, isUser } from '../lib/api/routeUtils';

// Define DocumentUser interface since it's not in the types/database.ts file
...
const documentDb = {
  ...
  getDocumentById: async (id: string): Promise<DocumentType> => {
  ...
  createDocument: async (userId: string, title: string, content: string): Promise<DocumentType> => {
  ...
  // Update a document
  updateDocument: async (id: string, updates: Partial<DocumentType>): Promise<DocumentType> => {
    // Placeholder implementation
  ...
  getUserDocuments: async (userId: string): Promise<Partial<DocumentType>[]> => {
 ...
 * @access  Private
 */
router.post('/', authenticate, asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Create document
    const document: DocumentType = await documentDb.createDocument(
      req.user!.id,
      title,
      content || ''
    );
...
router.get('/:id', authenticate, asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Get document
    const document: DocumentType = await documentDb.getDocumentById(documentId);
...
router.put('/:id', authenticate, asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Update document
    const document: DocumentType = await documentDb.updateDocument(documentId, { title, content });
 ...
 * @access  Private
 */
router.get('/:id/access', authenticate, asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const documentId = req.params.id;
    ...
    const documentUsers: Array<DocumentUser> = getDocumentUsers(documentId);

    // Fetch user details for each user ID
    const usersWithDetails = await Promise.all(
      documentUsers.map(async (docUser: DocumentUser) => {
        const userDetails = await userDb.getUserById(docUser.userId) as User | null;
        
        // Use the isUser type guard to ensure we have valid user data
 ...
 * @access  Private
 */
router.post('/:id/access', authenticate, asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const documentId = req.params.id;
 ...
 * @access  Private
 */
router.delete('/:id/access/:userId', authenticate, asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const documentId = req.params.id;
